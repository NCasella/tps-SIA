...
class VariationalAutoencoder:
    ...
    def encode(self, x: np.ndarray):
        ...
        h,h_partials = self.encoder.predict_output(x)
    ...
    def decode(self, z: np.ndarray) -> np.ndarray:
        x,_=self.decoder.predict_output(z)
        return x        
    ...
    def train(self, dataset, epochs):
        for epoch in range(epochs):
            ...
            for i in range(len(dataset)):
                ...
                # Encoder pass
                mu, log_var, enc_acts, enc_partials = self.encode(x)
                ...
                decoder_w = self.decoder.weights[0][:-1, :]  # skip decoder input bias
                dz = (x_hat - x) @ decoder_w.T

                dz_mu = dz + mu
                dz_logvar = 0.5 * (np.exp(log_var) - 1)
                dz_combined = np.hstack([dz_mu, dz_logvar]).reshape(1, -1)

                self.encoder.backpropagate(enc_partials, dz_combined, enc_acts)
